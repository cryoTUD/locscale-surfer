...
def extract_micelle(emdb_info, emdb_id, output_dir):
    try:
        output_file_path=os.path.join(output_dir, f"emd_{emdb_id}_micelle.mrc")
        if os.path.isfile(output_file_path):
            return emdb_id, output_file_path
        difference_mask_path = emdb_info["difference_mask_path"]
        pdb_path = emdb_info["aligned_pdb_path"]

        difference_mask, apix = load_map(difference_mask_path)
        ...
        membrane = get_membrane(N_coord, O_coord, difference_mask, apix, nr_membranes, imsize)
        membrane_extracted = difference_mask * membrane        
        
        output_file_path=os.path.join(output_dir, f"emd_{emdb_id}_micelle.mrc")
        
        save_as_mrc(membrane_extracted, output_file_path, apix)
        return emdb_id, output_file_path
...
emdb_info_full_json = "/home/abharadwaj1/scratch/dev/emmernet_training/segmentation_micelle_dataset_low_pass_filter/create_test_set/4_emdb_full_info_with_difference_mask.json"

micelle_folder = "/home/abharadwaj1/scratch/dev/segmentation_micelle/micelle_test_set_low_pass"
...
results = joblib.Parallel(n_jobs=n_jobs)(
    joblib.delayed(extract_micelle)\
    (emdb_info, emdb_id, micelle_folder) \
    for emdb_id, emdb_info in tqdm(emdb_info_full.items(), desc="Extracting micelle")
)
...
for emdb_id, output_file_path in results_filtered:
    ...
    emdb_info_full_with_micelle[emdb_id]["micelle_path"] = output_file_path

output_json_path = os.path.join(os.path.dirname(emdb_info_full_json), "5_emdb_full_info_with_micelle.json")
